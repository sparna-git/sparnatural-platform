...
export class SparqlReconcileServiceFactory {

  constructor() {    
  }

  createService(projectId:string, projectConfig:{sparqlEndpoint:string}, serviceConfig:{}): SparqlReconcileService {
    return new SparqlReconcileService(projectId, projectConfig.sparqlEndpoint);
  }
}
...
export class SparqlReconcileService implements ReconcileServiceIfc{
  ...
  private static CACHE_SIZE = 1000;
  ...
  async reconcileQueries(
    ...
    for (const [key, qobj] of Object.entries(queries)) {
      ...
      const uris = await this.runSparqlSearch(
        name,
        qobj.type,
        includeTypes
      );
      ...
      let results;
      if (includeTypes) {
        results = await this.formatResultsWithTypes(uris, name);
      } else {
        results = this.formatResults(uris, name);
      }
  ...
  updateCache(
    ...
    if (keys.length > SparqlReconcileService.CACHE_SIZE) {
      const oldestKey = keys.reduce((a, b) =>
        this.memoryCache[a].lastAccessed < this.memoryCache[b].lastAccessed ? a : b
      );
 ...
 async formatResultsWithTypes(
    ...
    for (let i = 0; i < sortedUris.length; i++) {
      ...
      const types = await this.getEntityTypes(uri, this.sparqlEndpoint);
      results.push({
        id: uri,
        name,
        type: types,
  ...
  async runSparqlSearch(
    ...
    let bindings: any[] = [];
    try {
      const url1 = `${this.sparqlEndpoint}?query=${encodeURIComponent(
        query1
      )}&format=json`;
      ...
      if (bindings.length === 0) {
        const query2 = `
          ...
          LIMIT ${SparqlReconcileService.MAX_RESULTS}
        `;
        const url2 = `${this.sparqlEndpoint}?query=${encodeURIComponent(
          query2
        )}&format=json`;
